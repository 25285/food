...
def init_db():
    ...
    conn.commit()
    conn.close()
    print("✅ SQLite Database initialized with your emails!")
...
def login(role):
    if request.method == 'POST':
        ...
        # 1. CHECK IF EMAIL IS IN DATABASE WHITELIST
        conn = get_db()
        user = conn.execute("SELECT * FROM users WHERE email=? AND role=?", (email, role)).fetchone()
        conn.close()

        if not user:
            return render_template('login.html', step="enter", role=role, error="Email not registered in the system.")
...
def generate_qr(event):
    ...
    if event == 'food' and current_hour < 13:
        return jsonify({"error": "Food Fest QR unlocks at 1:00 PM"}), 403
    if event == 'dj' and current_hour < 17:
        return jsonify({"error": "DJ Night QR unlocks at 5:00 PM"}), 403

    email = session['user']
    # Create new dynamic token
    token = f"{email}-{event}-{int(datetime.now().timestamp())}"
    
    conn = get_db()
    # Invalidate older unused tokens for this user/event to prevent hoarding
    conn.execute("UPDATE qr_codes SET status='expired' WHERE email=? AND event=? AND status='unused'", (email, event))
...
def validate():
    ...
    if qr_data['status'] == 'used':
        conn.close()
        return jsonify({"status": "Already Used"})

    # 30-SECOND DYNAMIC EXPIRY CHECK (We allow 35 seconds to account for internet delay)
    created_at = datetime.strptime(qr_data['created_at'], "%Y-%m-%d %H:%M:%S.%f")
    if datetime.now() - created_at > timedelta(seconds=35):
        conn.execute("UPDATE qr_codes SET status='expired' WHERE token=?", (token,))
        conn.commit()
        conn.close()
        return jsonify({"status": "Expired QR (Screenshot Detected)"})
